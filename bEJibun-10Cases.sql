...
FROM PurchaseTransaction PT JOIN PurchaseTransactionDetail PTD ON PT.PurchaseID=PTD.PurchaseID JOIN Item i ON i.ItemID=PTD.ItemID JOIN ItemType it ON it.ItemTypeID=i.ItemTypeID
...
FROM SalesTransaction ST JOIN SalesTransactionDetail STD ON ST.TransactionID=STD.TransactionID JOIN Item i ON i.ItemID=STD.ItemID JOIN ItemType it ON it.ItemTypeID=i.ItemTypeID
...
SELECT [Staff Number]=REPLACE(St.StaffID,'ST','Staff '), StaffName, [Salary]=CONCAT('Rp. ',StaffSalary), [Sales Count]=COUNT(DISTINCT Sls.TransactionID), [Average Sales Quantity]=AVG(SalesQty)
FROM Staff St JOIN SalesTransaction Sls ON St.StaffID=Sls.StaffID JOIN SalesTransactionDetail Std ON Std.TransactionID=Sls.TransactionID JOIN Customer c ON c.CustomerID=Sls.CustomerID
